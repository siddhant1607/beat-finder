...
def load_fingerprint_db():
    try:
        ...
        # Gracefully handle empty or corrupted pickle file
        ...
        if not query_hashes:
            ...
            if not song_time_diffs:
                ...
                for song_id, time_diffs in song_time_diffs.items():
                    grouped_diffs = [round(t / time_diff_tolerance) * time_diff_tolerance for t in time_diffs]
                    ...
                    if diff_counter:
                        ...
                        time_tightness = 1.0 / (1.0 + avg_deviation / 10.0)
                    else:
                        time_tightness = 0

                    confidence = fingerprinting.calculate_confidence(
                ...
                filtered_songs = {k: v for k, v in song_results.items() if v['dominant_matches'] >= match_threshold}

                if filtered_songs:
                    top_matches = sorted(filtered_songs.items(), key=lambda x: x[1]['confidence'], reverse=True)[:3]
